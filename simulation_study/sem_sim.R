...
# Example usage
# results <- evaluate_methods("AX", target_environment = 1, num_simulations = 1000)
# mean(results$rmse_global, na.rm = TRUE)
# mean(results$rmse_env_specific, na.rm = TRUE)


# library(ggplot2)
# 
# plot_results <- function(results, upper_lim=10) {
#   df <- data.frame(
#     SID = c(1:length(results$rmse_global),
#             1:length(results$rmse_global),
#             1:length(results$rmse_global)),
#     Model = c(rep("Global", length(results$rmse_global)), 
#               rep("Environment Specific", length(results$rmse_env_specific)),
#               rep("Weighted Global", length(results$rmse_weighted_global))),
#     RMSE = c(results$rmse_global, results$rmse_env_specific, results$rmse_weighted_global)
#   )
#   
#   ggplot(df, aes(x = Model, y = RMSE, fill = Model)) +
#     geom_line(aes(group = SID), alpha = 0.02, color = "gray20")+
#     geom_boxplot(aes(group = Model), width = 0.5, outlier.shape = NA)+
#     theme_minimal() +
...
# set.seed(123)
# num_simulations = 100
# results <- evaluate_methods(scenario_type = "AX",
#                             n_train = 100,
#                             n_target = 10,
#                             n_test = 50,
#                             n_cat = 10,
#                             num_env_predictors = 2,
#                             num_global_predictors = 2,
#                             target_environment = 1,
#                             from = -5,
#                             to = 5,
#                             num_simulations = num_simulations,
#                             overlap_threshold = NULL)
# plot_results(results,10)
# 
# sqrt(1/(num_simulations*(num_simulations-1))*sum((results$rmse_weighted_global-mean(results$rmse_weighted_global))^2))
# sqrt(1/(num_simulations*(num_simulations-1))*sum((results$rmse_global-mean(results$rmse_global))^2))
# sqrt(1/(num_simulations*(num_simulations-1))*sum((results$rmse_env_specific-mean(results$rmse_env_specific))^2))
...
# inspect_results <- param_grid_unnested
# Boxplots
# param_grid_unnested |> 
#   filter(scenario == "AY") |> 
#   ggplot(aes(x = as.factor(n_cat-1), y = RMSE, fill = as.factor(Method))) +
#   geom_boxplot() +
#   facet_grid(scenario ~ from, scales = "free_y") +
#   xlab("Number of External Environments") +
#   ylab("RMSE") +
#   labs(color = "Method")+
...
param_grid_unnested |> 
  ...
  # filter(to == 3) |>
  # filter(scenario == "AH") |> 
  group_by(scenario, n_target, ess_ratio, Method) |> 
  summarise(mean_rmse = mean(RMSE),
            lower_rmse = quantile(RMSE, 0.05)[[1]],
            upper_rmse = quantile(RMSE, 0.95)[[1]]) |> 
  ggplot(aes(ess_ratio, mean_rmse, color = as.factor(Method)))+
  geom_pointrange(aes(ymin = lower_rmse, ymax = upper_rmse),
                  alpha = 0.5,
                  position = position_jitter(width = 0.01))+
  # geom_jitter(alpha = 0.5, width = 0.001)+
  # geom_smooth(method = "loess", se = FALSE)+
  # geom_line(alpha = 0.3)+
  scale_color_brewer(palette = "Set1") +  # Set a color palette
  facet_grid(scenario~n_target, scales = "free", space = "free")+
  theme_minimal() +
  theme(
...
param_grid_unnested |> 
  ...
  # mutate(ess_ratio = as.factor(round(ess_ratio*2)/2)) |> 
  mutate(ess_ratio = as.factor(round(ess_ratio))) |> 
  # group_by(scenario, to_fct, ess_ratio = as.factor(round(ess_ratio*2)/2), Method) |> 
  ggplot(aes(x = ess_ratio, y = RMSE, fill = as.factor(Method))) +
  # ggplot(aes(x = as.factor(num_env_predictors), y = RMSE, fill = as.factor(Method), alpha = as.factor(Method))) +
  geom_boxplot(outlier.shape = NA) +  # Hide outliers
  # geom_jitter(width = 0.05, alpha = 0.5)+
  # geom_line(aes(group = SID),alpha = 0.4)+
  # scale_fill_lancet()+
  scale_fill_manual(values = c("#617989","#8f6b30", "#9447FB"), labels = c("Global","Local","Weighted")) +
  #scale_alpha_manual(values = c(0.4,0.7,1), guide = "none")+
  #scale_fill_discrete(labels = c("Local", "Global","Weighted"))+
  facet_grid(scenario~to_fct, scales = "free", space = "free") +
  # facet_grid(scenario~as.factor(num_env_predictors), scales = "free", space = "free") +
  theme_bw() +
  theme(
    axis.title = element_text(size = 16),
    axis.text = element_text(size = 14),
...
summary(lm(RMSE~scenario+n_cat+n_train+n_target+to+num_env_predictors, data = param_grid_unnested, subset = Method == "Weighted"))
...
#   # filter(scenario == "AY") |> 
#   filter(to == 2) |>
#   # ggplot(aes(x = as.factor(round(ess_ratio)), y = RMSE, fill = as.factor(Method))) +
#   ggplot(aes(x = as.factor(Method), y = RMSE, color = as.factor(round(ess_ratio)))) +
#   # geom_boxplot(outlier.shape = NA) +  # Hide outliers
#   geom_jitter(width = 0.05, alpha = 0.5)+
#   geom_line(aes(group = SID),alpha = 0.4)+
#   scale_color_brewer(palette = "Set1") +  # Set a color palette
